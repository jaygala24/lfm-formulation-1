...
parser.add_argument("--learning_rate_v", type=float, default=3e-4)
parser.add_argument("--weight_decay_v", type=float, default=1e-3)
parser.add_argument("--learning_rate_r", type=float, default=3e-4)
...
def main():
    ...
    arch, alphas_normal, alphas_reduce = initialize_alphas()

    if args.is_cifar100:
        model = Network(args.init_channels, CIFAR100_CLASSES, args.layers, criterion)
        model_pretrain = Network(
            args.init_channels, CIFAR100_CLASSES, args.layers, criterion
        )
    else:

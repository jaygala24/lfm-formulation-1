...
parser.add_argument(
    "--data", type=str, default="../data", help="location of data corpus"
)
parser.add_argument(
...
parser.add_argument(
    "--save", type=str, default="/tmp/checkpoints/", help="experiment path"
)
...
parser.add_argument(
    "--dropout_rate", action="append", default=[], help="dropout rate of skip connect"
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="search with cifar100 dataset",
)
...
def main():
    ...
    for sp in range(len(num_to_keep)):
        # actual model
        model = Network(
            args.init_channels + int(add_width[sp]),
            CIFAR_CLASSES,
        ...
        arch, alphas_normal, alphas_reduce = initialize_alphas(model)

        # share the same architecture
        model.module._arch_parameters = arch
        model.module.alphas_reduce = alphas_reduce
        model.module.alphas_normal = alphas_normal
        model_pretrain.module._arch_parameters = arch
        model_pretrain.module.alphas_reduce = alphas_reduce
        model_pretrain.module.alphas_normal = alphas_normal
        ...
        for epoch in range(epochs + args.pretrain_steps):
            ...
            if epoch < args.pretrain_steps:
                ...
                model_pretrain.module.p = (
                    float(drop_rate[sp])
                    * (epochs + args.pretrain_steps - epoch - 1)
                    / (epochs + args.pretrain_steps)
                )
                ...
                if epoch < eps_no_arch + args.pretrain_steps:
                    ...
                    model.module.p = (
                        float(drop_rate[sp])
                        * (epochs - epoch + args.pretrain_steps - 1)
                        / epochs
                    )
                    ...
                    model.module.p = float(drop_rate[sp]) * np.exp(
                        -(epoch - args.pretrain_steps - eps_no_arch) * scale_factor
                    )
                    model.module.update_p()
                    # model for pretraining.
                    model_pretrain.module.p = float(drop_rate[sp]) * np.exp(
                        -(epoch - eps_no_arch - args.pretrain_steps) * scale_factor
                    )
                    model_pretrain.module.update_p()
            ...
            if (
                ...
                and (epochs + args.pretrain_steps) - epoch < 5
            ):
...
def train(
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if epoch >= args.pretrain_steps:
            assert (
                model_pretrain.module._arch_parameters[0]
                - model.module._arch_parameters[0]
            ).sum() == 0
            assert (
                model_pretrain.module._arch_parameters[1]
                - model.module._arch_parameters[1]
            ).sum() == 0
